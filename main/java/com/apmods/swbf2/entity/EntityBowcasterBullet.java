...
public class EntityBowcasterBullet extends EntityBullet implements IProjectile{
	
	private EntityLivingBase shootingEntity;
	
	private float damage = 0;
	
	public EntityBowcasterBullet(World par1World) {
	       super(par1World);
	       this.setThrowableHeading(motionX, motionY, motionZ, 4.5f, 1F);
	   }
	   public EntityBowcasterBullet(World par1World, EntityLivingBase entity, float speed) {
	       super(par1World, entity, speed);
	       this.shootingEntity = entity;
	       this.setThrowableHeading(motionX, motionY, motionZ, speed, damage);
	   }
	   public EntityBowcasterBullet(World par1World, EntityPlayer player, float speed, float offset) {
	       super(par1World, player, speed);
	       this.shootingEntity = player;
	       Vec3 v = player.getLookVec().normalize();
	       damage = 0.5f;
	       this.setPosition( player.posX + v.xCoord * offset,  player.posY + player.eyeHeight - 0.3f + v.yCoord * offset,  player.posZ + v.zCoord * offset);
	       this.setThrowableHeading(motionX, motionY, motionZ, speed, 1.0f);
	   }
	   public EntityBowcasterBullet(World par1World, EntityLivingBase par2EntityLivingBase, EntityLivingBase par3EntityLivingBase, float par4, int par5)
	   {
	       super(par1World);
